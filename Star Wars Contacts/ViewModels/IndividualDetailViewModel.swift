...
class IndividualDetailViewModel: ObservableObject {
    ...
    var id: ID { model.id }
    var birthdate: String { model.birthdate }
    ...
    func getImageOld() -> CGImage {
        return imageStore.getImage(for: imageID) ?? Theme.defaultImage
    }
    func getImage(for imageID: String) -> CGImage {
        print("imageId is: \(imageID)")
        if let fetchedImage = imageStore.getImage(for: imageID){
               return fetchedImage
           } else {
               // Fallback to local image saved in assets
               if let localImage = UIImage(named: imageID)?.cgImage {
                       return localImage
                   } else {
